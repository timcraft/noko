...
class FreckleClientExpensesTest < FreckleClientTest
  ...
  def test_get_expense
    ...
    assert_instance_of Freckle::Record, client.get_expense(id)
  end

  def test_create_expense
    expect_request(:post, "#{base_url}/expenses").with(json_request).to_return(json_response.merge(status: 201))

    assert_instance_of Freckle::Record, client.create_expense(date: Date.today, project_id: id, price: '14.55')
  end

  def test_update_expense
    expect_request(:put, "#{base_url}/expenses/#{id}").with(json_request).to_return(json_response)

    assert_instance_of Freckle::Record, client.update_expense(id, price: '19.99')
  end
