...
class FreckleClientInvoicesTest < FreckleClientTest
  ...
  def test_get_invoice
    ...
    assert_instance_of Freckle::Record, client.get_invoice(id)
  end

  def test_create_invoice
    expect_request(:post, "#{base_url}/invoices").with(json_request).to_return(json_response.merge(status: 201))

    assert_instance_of Freckle::Record, client.create_invoice(invoice_date: Date.today)
  end

  def test_update_invoice
    expect_request(:put, "#{base_url}/invoices/#{id}").with(json_request).to_return(json_response)

    assert_instance_of Freckle::Record, client.update_invoice(id, reference: 'AB 0001')
  end
