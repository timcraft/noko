...
class FreckleClientProjectsTest < FreckleClientTest
  ...
  def test_get_project
    ...
    assert_instance_of Freckle::Record, client.get_project(id)
  end

  def test_create_project
    expect_request(:post, "#{base_url}/projects").with(json_request).to_return(json_response.merge(status: 201))

    assert_instance_of Freckle::Record, client.create_project(name: 'Gear GmbH')
  end
  ...
  def test_update_project
    ...
    assert_instance_of Freckle::Record, client.update_project(id, color: '#ff9898')
  end
